...
class TaskDAO(val context: Context) {
    ...
    fun insert(task: Task) {
        ...
        val values = ContentValues().apply {
            put(Task.COLUM_NAME, task.name)
            put(Task.COLUMN_DONE, task.done)
        }

        try {
            // Insert the new row, retunring the primary key value of the new row
    ...
    fun delete(task: Task) {
        ...
        try {
            ...
            val deleteRows = db.delete(Task.TABLE_NAME, "${Task.COLUMN_ID} = ${task.id}", null)
